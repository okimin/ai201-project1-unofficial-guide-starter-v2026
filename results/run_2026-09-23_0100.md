...
```
Walk-in hours for the health centre are from 8am to 11am. 
...
```
Based on the provided documents, **The Atrium** has a wait time of "no queue" (dining_the_atrium.txt). Additionally, **North Kitchen** also has a wait figure of none (dining_north_kitchen_followup.txt).
```
...
```
Both The Atrium and North Kitchen have no waiting queue, according to the documents (`dining_the_atrium.txt`, `dining_the_atrium_followup.txt`, and `dining_north_kitchen_followup.txt`).
```
